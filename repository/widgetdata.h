...
class WidgetData : public QObject, public MixinTreeNode<WidgetData>,
        protected AttrContainer, public XmlData, public UniqueId
{
    Q_OBJECT
...
public:
    ...
    template<typename T> inline T getAttr(int key) const {
        return AttrContainer::getAttr<T>(key);
    }
    template<typename T> bool setAttr(int key, const T &value) {
...
//    template<typename T>
//    inline bool setHashMember(QHash<int,T> &hash, int k, const T &value, bool add = false) {
//        auto it = hash.find(k);
//        if (it != hash.end()) {
//            *it = value;
//            emit attrChanged(k);
